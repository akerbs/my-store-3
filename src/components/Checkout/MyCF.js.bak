...
const schema = yup.object().shape({
  ...
  cardDuration: yup.string().required("Field is required"),
  // .matches(/^[0-9\-\+]{9,15}$/, "incorrect phone number"), // CHANGE!!!!
  cvvCvc: yup.string().required("Field is required"),
  // .matches(/^[0-9\-\+]{9,15}$/, "incorrect phone number"), // CHANGE!!!!
  name: yup
    .string()
    // .matches(/^[0-9\-\+]{9,15}$/, "incorrect phone number") // CHANGE!!!!
...
export default function MyCheckoutForm(props) {
  ...
  const { cart, ttlPrice, currentCurrency } = useContext(CartContext)
  ...
  const errorCvvCvc =
    errors.hasOwnProperty("cvvCvc") && errors["cvvCvc"].message
